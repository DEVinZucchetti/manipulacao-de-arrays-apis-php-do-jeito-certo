...
if ($method === 'POST') {
    ...
    $data = [
        'id' => $_SERVER['REQUEST_TIME'], // SOMENTE PARA ESSE EXERCICIO, POIS ESTOU PEGANDO A HORA EXATA
        'name' => $name,
        'contact' => $contact,
...
} elseif ($method === 'PUT') { // Atualizar uma informação
    $body = getBody();
    $id = filter_var($_GET['id'], FILTER_VALIDATE_INT);
    ...
    saveFileContent(FILE_COUNTRY, $allData);
    response(['message' => 'Informação atualizada com sucesso'], 200);
}
